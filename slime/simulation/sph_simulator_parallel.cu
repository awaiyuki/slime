...
__device__ int3 toCellPosition(const float3 &position) {
  return make_int3(floor((position.x + 1.0f) / SMOOTHING_RADIUS),
                   floor((position.y + 1.0f) / SMOOTHING_RADIUS),
                   floor((position.z + 1.0f) / SMOOTHING_RADIUS));
}
...
__device__ float gradientSpikyKernelDevice(float3 r, float h) {
  ...
  if (rMagnitude > h || rMagnitude < EPSILON)
    return 0.0f;
...
__global__ void slime::g_computeDensity(Particle *d_particles,
  ...
  i.density = i.mass * poly6KernelDevice(make_float3(0.0f, 0.0f, 0.0f),
                                         SMOOTHING_RADIUS);
  ...
  for (int nx = -1; nx <= 1; nx++) {
    for (int ny = -1; ny <= 1; ny++) {
      for (int nz = -1; nz <= 1; nz++) {
        int3 neighborCell =
            make_int3(cellPosition.x + nx, cellPosition.y + ny,
                      cellPosition.z + nz);
        unsigned int cellHash = hash(neighborCell);
        unsigned int key = keyFromHash(cellHash);
        if (bucketStart[key] == UINT_MAX || bucketEnd[key] == UINT_MAX)
          continue;
        const unsigned int end =
            min(bucketEnd[key], bucketStart[key] + MAX_NEIGHBORS);
        for (int bucketIdx = bucketStart[key]; bucketIdx < end;
             bucketIdx++) {
...
__global__ void slime::g_computePressure(Particle *d_particles) {
  ...
  if (idx >= NUM_PARTICLES)
    return;
  auto &i = d_particles[idx];
  i.pressure = GAS_CONSTANT * fmaxf(i.density - REST_DENSITY, 0.0f);
}

__global__ void slime::g_computeForces(Particle *d_particles,
                                       float3 *nextVelocities,
                                       unsigned int *hashIndices,
                                       unsigned int *bucketStart,
                                       unsigned int *bucketEnd,
                                       double deltaTime) {
  ...
  acceleration +=
      (pressureForce + viscosityForce) / fmaxf(pi.density, REST_DENSITY);
  ...
  if (normGrad > SURFACE_NORMAL_THRESHOLD) {
    ...
    acceleration += surfaceTensionForce / fmaxf(pi.density, REST_DENSITY);
  }

  const float accelerationMagnitude = length(acceleration);
  if (!isfinite(accelerationMagnitude)) {
    nextVelocities[idx] = make_float3(0.0f, 0.0f, 0.0f);
    return;
  }
  if (accelerationMagnitude > MAX_ACCELERATION)
    acceleration *= MAX_ACCELERATION / accelerationMagnitude;

  auto deltaVelocity = acceleration * static_cast<float>(deltaTime);
  float3 nextVelocity = pi.velocity + deltaVelocity;
  nextVelocity *= VELOCITY_DAMPING;
  const float speed = length(nextVelocity);
  if (speed > MAX_SPEED)
    nextVelocity *= MAX_SPEED / speed;
  nextVelocities[idx] = nextVelocity;
}

__global__ void slime::g_applyVelocities(Particle *d_particles,
                                         float3 *nextVelocities) {
  int idx = threadIdx.x + blockDim.x * blockIdx.x;
  if (idx >= NUM_PARTICLES)
    return;
  d_particles[idx].velocity = nextVelocities[idx];
}
